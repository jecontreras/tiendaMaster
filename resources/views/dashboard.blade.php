        ...
        <div class="media-container-row align-center">
            <div class="col-12 col-md-12">
                ...
                <div id="bootstrap-accordion_29" class="panel-group accordionStyles accordion col-sm-12 col-md-10 col-lg-8 align-left pt-5" role="tablist" aria-multiselectable="true">
                    @foreach(App\Order::where('user_id', auth()->user()->id)->orderBy('id', 'desc')->get() as $order)
                    <div class="card">
                        <div class="card-header" role="tab" id="headingOne">
                            <a role="button" class="collapsed panel-title text-black" href="@if($order->payu_order_id)/order/{{$order->payu_order_id}}@else#!@endif" >
                                ...
                                <h4 class="mbr-fonts-style header-text display-5">
                                  {{$order->created_at}} - @if($order->payu_order_id){{($order->reference=="APPROVED"?'PAGADA':$order->reference)}}@else INCOMPLETA @endif
                                </h4>
                            </a>
